...
async function requireTeacher(userId) {
    const { rows } = await pool.query(`SELECT role FROM students WHERE id = $1`, [userId]);
    const role = rows[0]?.role;
    return role === 'teacher' || role === 'admin';
}
export async function teacherRoutes(app) {
    // GET /api/teacher/students — lista todas alunas com stats
    app.get('/api/teacher/students', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        try {
            const [studentsResult, gamResult, progressResult] = await Promise.all([
                pool.query(`SELECT id, email, name, role, created_at
             FROM students
             ORDER BY created_at DESC`),
                pool.query(`SELECT student_id, xp, level, current_streak, last_active_date
             FROM gamification`),
                pool.query(`SELECT student_id, COUNT(*) as completed_count
             FROM lesson_progress
             WHERE status = 'completed'
             GROUP BY student_id`),
            ]);
            const gamMap = new Map(gamResult.rows.map((g) => [g.student_id, g]));
            const progressMap = new Map(progressResult.rows.map((p) => [
                p.student_id,
                parseInt(p.completed_count, 10),
            ]));
            const students = studentsResult.rows.map((s) => {
                const g = gamMap.get(s.id);
                return {
                    id: s.id,
                    email: s.email,
                    name: s.name,
                    ...
                    completedLessons: progressMap.get(s.id) ?? 0,
                    xp: g?.xp ?? 0,
                    level: g?.level ?? 1,
    ...
    app.get('/api/teacher/students/:id/gamification', { preHandler: authMiddleware }, async (request, reply) => {
        const { userId } = request;
