...
interface CompleteLessonBody {
  ...
  newAchievements: Array<{ id: string; unlocked_at: string }>
}
...
export async function lessonsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: CompleteLessonBody }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        await client.query(
          ...
          [userId, lessonId, status, JSON.stringify(response)],
        )
        ...
        await client.query(
          `INSERT INTO gamification (student_id, xp, level, daily_activity, last_active_date, current_streak)
           ...
           DO UPDATE SET
             ...
             current_streak = $4`,
          [userId, xpDelta, JSON.stringify(dailyActivity), currentStreak],
        )
  ...
  app.put<{ Body: DraftBody }>(
    ...
    async (request, reply) => {
      ...
      try {
        await pool.query(
          `INSERT INTO lesson_progress (student_id, lesson_id, status, response, saved_at)
           VALUES ($1, $2, 'in_progress', $3, now())
           ON CONFLICT (student_id, lesson_id)
           DO UPDATE SET
             response = EXCLUDED.response,
             saved_at = EXCLUDED.saved_at`,
          [userId, lessonId, JSON.stringify(response)],
        )
        return reply.status(200).send({ ok: true })
      } catch (err) {
